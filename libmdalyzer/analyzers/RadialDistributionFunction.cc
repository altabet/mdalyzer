...
RadialDistFunc::RadialDistFunc(boost::shared_ptr<Trajectory> traj, const std::string& file_name, const double& delR, const double& maxR, int& nSkip)
   : Analyzer(traj), m_file_name(file_name), m_delR(delR), m_maxR(maxR), m_nSkip(nSkip)
   {
   }

inline Vector3<double>  RadialDistFunc::wrapCoords(Vector3<double> cur_pos, const Vector3<double> box_len, const Vector3<double> inv_box_len) {
...
void RadialDistFunc::evaluate() {  // need to add option to do g2(r) of specific types e.g. AA AB BB
   ...
   if (!frames[0]->hasBox()) {
	...
	if (!cur_frame->hasBox()) {
           // error! box not found
	   throw std::runtime_error("RadialDistFunc needs a simulation box in all frames");
	}
...
void export_RadialDistFunc() {
   ...
   ("RadialDistFunc", init< boost::shared_ptr<Trajectory>, const std::string&, const double&, const double&, int& >()); 
}
